Public Class clsPRE
    ...
    Public Sub LoadScreen()
        objForm = objAddOn.objUIXml.LoadScreenXML("Prepaid.xml", Mukesh.SBOLib.UIXML.enuResourceType.Embeded, Formtype)
        objForm.AutoManaged = True
        objRecordSet = objAddOn.objCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        strSQL = "select Code,Name,U_AppType from [@MIPLEX] where U_PrePro='PE'"
        ...
        While Not objRecordSet.EoF
            ...
            objComboBox = objForm.Items.Item("6B").Specific
        ...
        objForm.Items.Item("10").Specific.Active = True
        objForm.Items.Item("10").Specific.String = "A"
        PPELine = objForm.DataSources.DBDataSources.Item("@MIPLPREP1")
        PPEHeader = objForm.DataSources.DBDataSources.Item("@MIPLPREP")
    End Sub
        ...
        Else
            Select Case pVal.EventType
                ...
                Case SAPbouiCOM.BoEventTypes.et_COMBO_SELECT
                    If pVal.ItemUID = "6" And objForm.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE Then
                        LoadLedger(FormUID)
                    End If
                Case SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST
                    If pVal.ItemUID = "12" And (pVal.ColUID = "2" Or pVal.ColUID = "4") Or pVal.ItemUID = "4" Then
                        CFL(FormUID, pVal)
                    End If
                ...
                Case SAPbouiCOM.BoEventTypes.et_LOST_FOCUS
                    ...
                    If pVal.ItemUID = "10" Then
                        ...
                        AddNewRow(FormUID)
                    ElseIf pVal.ItemUID = "12" And pVal.ColUID = "8" Then
                        CalcTotalDays(FormUID)
                        AddNewRow(FormUID)
                        'objMatrix.AddRow(1, pVal.Row)
                    End If
            End Select
    ...
    Private Sub LoadLedger(ByVal FormUID As String)
        objForm = objAddOn.objApplication.Forms.Item(FormUID)
        objMatrix = objForm.Items.Item("12").Specific
        strSQL = "select Code,Name,U_CreditGL,U_CreditGL1,U_DebitGL,U_DebitGL1 from [@MIPLEX]  where Code='" & objForm.Items.Item("6").Specific.selected.value & "'"
        objRecordSet.DoQuery(strSQL)
        objMatrix.Clear()
        objForm.Items.Item("4").Specific.String = objRecordSet.Fields.Item("U_CreditGL").Value
        objForm.Items.Item("11").Specific.String = objRecordSet.Fields.Item("U_CreditGL1").Value
        While Not objRecordSet.EoF
            objMatrix.AddRow()
            PPELine.Clear()
            objMatrix.GetLineData(objMatrix.RowCount)
            PPELine.SetValue("U_CreditGL", 0, objRecordSet.Fields.Item("U_CreditGL").Value)
            PPELine.SetValue("U_CreditGL1", 0, objRecordSet.Fields.Item("U_CreditGL1").Value)
            PPELine.SetValue("U_DebitGL", 0, objRecordSet.Fields.Item("U_DebitGL").Value)
            PPELine.SetValue("U_DebitGL1", 0, objRecordSet.Fields.Item("U_DebitGL1").Value)
    ...
    Private Function validate(ByVal FormUID As String) As Boolean
        ...
        If objMatrix.RowCount = 0 Then
            ...
            Return False
        ElseIf objForm.Items.Item("6B").Specific.String = "" Or objMatrix.Columns.Item("1").Cells.Item(1).Specific.String = "" Or objMatrix.Columns.Item("6").Cells.Item(1).Specific.String = "" Or objMatrix.Columns.Item("7").Cells.Item(1).Specific.String = "" Or objMatrix.Columns.Item("8").Cells.Item(1).Specific.String = "" Then
            ...
            Select Case objCFLEvent.ChooseFromListUID
                Case "CFL_1"
                    Try
                        ...
                        Else
                            objForm.Items.Item("4").Specific.String = objDataTable.GetValue("AcctCode", 0)
                ...
                Case "CFL_2"
                    Try
                        If objDataTable Is Nothing Then
                        Else
                            objMatrix.Columns.Item("2").Cells.Item(pval.Row).Specific.String = objDataTable.GetValue("AcctCode", 0)
                            objMatrix.Columns.Item("3").Cells.Item(pval.Row).Specific.String = objDataTable.GetValue("AcctName", 0)
                        End If
                    Catch ex As Exception
                        objMatrix.Columns.Item("3").Cells.Item(pval.Row).Specific.String = objDataTable.GetValue("AcctName", 0)
                        objMatrix.Columns.Item("2").Cells.Item(pval.Row).Specific.String = objDataTable.GetValue("AcctCode", 0)
                ...
                Case "CFL_3"
                    Try
                        If objDataTable Is Nothing Then
                        Else
                            objMatrix.Columns.Item("4").Cells.Item(pval.Row).Specific.String = objDataTable.GetValue("AcctCode", 0)
                            objMatrix.Columns.Item("5").Cells.Item(pval.Row).Specific.String = objDataTable.GetValue("AcctName", 0)
                        End If
                    Catch ex As Exception
                        objMatrix.Columns.Item("5").Cells.Item(pval.Row).Specific.String = objDataTable.GetValue("AcctName", 0)
                        objMatrix.Columns.Item("4").Cells.Item(pval.Row).Specific.String = objDataTable.GetValue("AcctCode", 0)
    ...
    End Sub
    Private Sub CalcTotalDays(ByVal FormUID As String)
        objForm = objAddOn.objApplication.Forms.Item(FormUID)
        PPELine = objForm.DataSources.DBDataSources.Item("@MIPLPREP1")
        objMatrix = objForm.Items.Item("12").Specific
        Dim startdate As Date, enddate As Date
        Dim noofdays As Integer, i As Integer
        objForm.Items.Item("8").Specific.String = objAddOn.objGenFunc.GetDocNum(Formtype)
        For i = 1 To objMatrix.RowCount
            startdate = objAddOn.objGenFunc.GetDateTimeValue(objMatrix.Columns.Item("7").Cells.Item(i).Specific.String)
            enddate = objAddOn.objGenFunc.GetDateTimeValue(objMatrix.Columns.Item("8").Cells.Item(i).Specific.String)
            noofdays = CInt(DateDiff(DateInterval.Day, startdate, enddate))
            objMatrix.Columns.Item("9").Cells.Item(i).Specific.String = noofdays
        Next i
    End Sub
